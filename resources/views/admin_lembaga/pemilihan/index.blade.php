...
@section('content')
    <div class="bgc-white bd bdrs-3 pB-50">
        ...
        <a href="{{ route('admin_lembaga' . '.pemilihan.create') }}" class="btn btn-info pull-right mR-10 mT-5">
        ...
        <table id="dataTable" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
                @foreach ($items as $item)
                    <tr>
                        <td><a href="{{ route('admin_lembaga' . '.pemilihan.edit', $item->id) }}">{{ $item->id }}</a></td>
                        <td>{{ $item->jenis }}</td>
                        <td>{{ $item->tahun }}</td>
                        <td>    
                                {{ $item->provinsi->nama }}
                        ...
                        <td>
                            <ul class="list-inline">
                                ...
                                <li class="list-inline-item">
                                    {!! Form::open([
                                        'class'=>'delete',
                                        'url'  => route('admin_lembaga' . '.pemilihan.destroy', $item->id), 
                                        'method' => 'DELETE',
                                        ]) 
                                    !!}
                                ...
                                <li class="list-inline-item">
                                    <a href="{{ url('/admin_lembaga/pemilihan/saksi', $item->id) }}" class="btn btn-primary btn-sm" title="{{ trans('Saksi TPS') }}"><span class="fa fa-user"></span></a>
                                </li>
                                <li class="list-inline-item">
                                    <a href="{{ url('/admin_lembaga/pemilihan/tps', $item->id) }}" class="btn btn-primary btn-sm" title="{{ trans('Data TPS') }}"><span class="fa fa-address-card"></span></a>
                                </li>
                            </ul>
                        </td>
                    </tr>
